...
mod scalar_solution {
    ...
    fn process_row<const HAS_ABOVE: bool, const HAS_BELOW: bool>(
        ...
        for c in 1..(cols - 1) {
            ...
            let mut sum =
                (curr[c - 1] == b'@') as u8 + (curr[c + 1] == b'@') as u8;
    ...
    pub fn parse(input: &str) -> (Vec<(usize, usize)>, Array2<u8>) {
        ...
        let count_grid = unsafe {
            Array2::from_shape_vec_unchecked((rows, cols), count_data)
        };
    ...
    pub fn part2(input: &Input) -> usize {
        ...
        while let Some(pos) = todo.pop() {
            ...
            OFFSETS.into_iter().for_each(|offset| {
                let new = (
                    pos.0.wrapping_add(offset.0),
                    pos.1.wrapping_add(offset.1),
                );
...
mod simd_solution {
    ...
    fn process_row<const HAS_ABOVE: bool, const HAS_BELOW: bool>(
        ...
        while c < end_simd {
            ...
            // Find todo values
            let mut todo_mask =
                (mask & sums.simd_lt(Simd::splat(4))).to_bitmask();
            while todo_mask != 0 {
...
#[cfg(not(feature = "simd"))]
pub use scalar_solution::{parse, part1, part2};
#[cfg(feature = "simd")]
pub use simd_solution::{parse, part1, part2};
